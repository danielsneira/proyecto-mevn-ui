<template>
  <v-data-table :headers="headers" :items="ventas" class="elevation-1">
    ...
    <template v-slot:no-data>
      <v-btn color="primary" @click="listarVentas">
        Reset
      </v-btn>
    </template>
  </v-data-table>
...
export default {
  ...
  methods: {
    ...
    activateItem() {
      const { token } = this.$store.state
      axios
        .put(`/venta/activar/${this.editedItem._id}`, this.editedItem, {
          headers: { token }
        })
        .then(response => {
          console.log(response.data)
    ...
    deactivateItem() {
      const { token } = this.$store.state
      axios
        .put(`/venta/desactivar/${this.editedItem._id}`, this.editedItem, {
          headers: { token }
        })
        .then(response => {
          console.log(response.data)
